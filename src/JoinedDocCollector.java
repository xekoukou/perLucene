...
return;
  ...
  public  JoinedDocCollector(int numHits,VarInt intersection, boolean docsScoredInOrder) {
    
    if (numHits <= 0) {
      throw new IllegalArgumentException("numHits must be > 0; please use TotalHitCountCollector if you just need the total hit count");
    }
    if(!docsScoredInOrder){
throw new IllegalArgumentException("Only queries that score in sorted order are supported");
